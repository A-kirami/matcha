...
type Strategy<R> = Record<string, ((context: object) => R) | undefined>
...
export abstract class AdapterMessageHandler<M extends Message> {
  ...
  async build(contents: Contents[]): Promise<M[]> {
    ...
    const messages = await Promise.all(collect)
    return messages.filter(e => nonNullable(e))
  }
  ...
  async parse(messages: M[]): Promise<Contents[]> {
    ...
    const contents = await Promise.all(collect)
    return contents.filter(e => nonNullable(e))
  }
